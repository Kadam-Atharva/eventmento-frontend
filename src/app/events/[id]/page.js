        ...
        return (
            <div className="min-h-screen flex justify-center items-center bg-gray-50">
                <div className="text-center">
                    ...
                    <button onClick={() => router.push('/dashboard')} className="mt-4 px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                        Go to Dashboard
                    </button>
                </div>
            </div>
            ...
            <div className="relative h-96 w-full bg-gray-900">
                ...
                <div className="absolute inset-0 flex flex-col justify-end p-8 md:p-16 bg-gradient-to-t from-gray-900 via-transparent to-transparent">
                    <div className="max-w-7xl mx-auto w-full">
                        <span className={`inline-block px-3 py-1 mb-4 text-xs font-bold tracking-wider uppercase rounded-full ${
                             event.status === 'PUBLISHED' ? 'bg-green-500 text-white' : 'bg-yellow-500 text-white'
                        }`}>
                            {event.status}
                        </span>
                        <h1 className="text-4xl md:text-6xl font-extrabold text-white mb-4 shadow-sm">{event.name}</h1>
                        <div className="flex flex-col md:flex-row gap-4 md:gap-8 text-gray-200">
                            <div className="flex items-center">
                                ...
                                <span className="text-lg font-medium">{event.start ? new Date(event.start).toLocaleString() : 'Date TBD'}</span>
                            </div>
                            <div className="flex items-center">
                                ...
                                {event.ticketTypes.map((ticket) => (
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <span className="font-bold text-lg text-blue-600">${ticket.price}</span>
                                        </div>
                                    ...
                                    <div className="mt-6 pt-6 border-t border-gray-100 animate-fadeIn">
                                        <div className="flex justify-between items-center mb-4">
                                            ...
                                            <div className="flex items-center space-x-3">
                                                ...
                                                <button 
                                                    onClick={() => setQuantity(Math.min(10, quantity + 1))} // Cap at 10 for safety
                                                    className="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-600 font-bold transition"
                                                >
                                                    +
                                                </button>
                                        ...
                                        <div className="flex justify-between items-center mb-6 pt-4 border-t border-gray-100">
                                            ...
                                            <span className="text-2xl font-bold text-blue-600">${(selectedTicket.price * quantity).toFixed(2)}</span>
                                        </div>
                                        ...
                                        >
                                            {bookingLoading ? 'Processing...' : (auth.isAuthenticated ? 'Book Tickets' : 'Login to Book')}
                                        </button>
                                    </div>
                                )}
                            </div>
